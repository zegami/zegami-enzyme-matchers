...
function single(matcherFn) {
  return function singleWrapper(enzymeWrapper) {
    ...
    // Preserve utilities set inside this for the matchers

    for (var _len = arguments.length, args = Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
      args[_key - 1] = arguments[_key];
    }

    return matcherFn.call.apply(matcherFn, [this, enzymeWrapper].concat(args)); // eslint-disable-line prefer-reflect
  };
} /*
   * Copyright 2016 hudl
   * All rights reserved.
